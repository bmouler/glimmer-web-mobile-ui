...
if ($ref_file eq "") {
    ...
    open ( UPLOADFILE2, ">$upload_dir/REF_$ref_file.fasta" ) or die "$!";
    binmode UPLOADFILE2;
    $ref_file = "REF_$ref_file";
...
my $shellcmd1 = 'PATH=$PATH:/var/www/bmouler/final/glimmer3/bin && cd /var/www/bmouler/final/uploads && '
    ."$long_orfs_cmd && $extract1_cmd && $build_icm_cmd && $glimmer3_cmd && $extract2_cmd";
system($shellcmd1);
...
############################################################
##########Clean up glimmer output files
############################################################

my $shellcmd2 = 'cd /var/www/bmouler/final/uploads && '.
    "[ -f $query_file\.longorfs ] && rm $query_file\.longorfs ; "
    ."[ -f $query_file\.train ] && rm $query_file\.train ; "
    ."[ -f $query_file\.predict ] && rm $query_file\.predict ; "
    ."[ -f $query_file\.icm ] && rm $query_file\.icm ; "
    ."[ -f $query_file\.detail ] && rm $query_file\.detail ; "
    ."[ -f $query_file\.fasta ] && rm $query_file\.fasta ; "
    ."[ -f $query_file\.glimmer ] && rm $query_file\.glimmer";
system($shellcmd2);

if ($ref_file ne $query_file) {
    my $shellcmd3 = 'cd /var/www/bmouler/final/uploads && '
        ."[ -f $ref_file\.longorfs ] && rm $ref_file\.longorfs ; "
        ."[ -f $ref_file\.train ] && rm $ref_file\.train ; "
        ."[ -f $ref_file\.fasta ] && rm $ref_file\.fasta ; "
        ."[ -f $ref_file\.icm ] && rm $ref_file\.icm";
    system($shellcmd3);
}
